// cpp/clisoap2/main.cpp
// Uso: ./clisoap2.exe  -> http://localhost:7002/?n=10
// Consume el servicio SOAP y traduce al español
...
std::string translate(const std::string& text) {
    ...
    std::string readBuffer;
    std::string url = "https://translate.googleapis.com/translate_a/single?client=gtx&sl=en&tl=es&dt=t&q=" + text;
    ...
    if (res == CURLE_OK) {
        std::regex pattern("\\[\\[\\\"(.*?)\\\"");
        std::smatch match;
        if (std::regex_search(readBuffer, match, pattern)) {
            return match[1].str();
        }
    }
    return text;
...
int main() {
    ...
    svr.Get("/", [](const httplib::Request& req, httplib::Response& res) {
        ...
        std::string en = call_soap(n);
        std::string es = translate(en);
        res.set_content(es, "text/plain; charset=utf-8");
    });

    std::cout << "http://localhost:7002/?n=10" << std::endl;
    svr.listen("0.0.0.0", 7002);
